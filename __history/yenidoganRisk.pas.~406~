...
OpenHafta(qrHafta4, 4);
 ...
 procedure TForm2.ApplyTGFieldRules;
var
  I: Integer;
  C: TComponent;
begin
...
procedure TForm2.btnYazdirClick(Sender: TObject);
var
  FormNo: Integer;
begin
  // gridde seçim var mı?
...
begin
  ...
  // 2️⃣ Hasta bilgisi (ÖNCE)
  FDosyaNo    := TEST_DOSYA_NO;
  FProtokolNo := TEST_PROTOKOL_NO;

  // 3️⃣ 🔹 GEÇMİŞ FORM LİSTESİ (DBGrid için)
  qryFormHistory.Close;
  qryFormHistory.ParamByName('DOSYA_NO').AsInteger := FDosyaNo;
  qryFormHistory.ParamByName('PROTOKOL_NO').AsInteger := FProtokolNo;
  qryFormHistory.Open;

  // 4️⃣ Aktif form / hafta hesapla
  CalcAktifFormHafta;
...
procedure TForm2.ClearRiskFieldsInDatasetExcept(
  DS: TDataSet;
