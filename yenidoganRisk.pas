...
type
  TForm2 = class(TForm)
    ...
    procedure EnsureHaftaRecord(Q: TOraQuery; AHafta: Integer);
...
{$R *.dfm}
procedure TForm2.EnsureHaftaRecord(Q: TOraQuery; AHafta: Integer);
begin
  if Q.IsEmpty then
  begin
    Q.Append;
    ...
    // KAYDI AÇIK BIRAKIYORUZ, POST YOK!
  end
  else
  begin
...
begin
  ...
  if (qrrastgeleHasta.FieldByName('YR_28_HAFTA').AsString = 'T')
  or (qrrastgeleHasta.FieldByName('YR_1000_GR').AsString = 'T')
  or (qrrastgeleHasta.FieldByName('YR_NEK_GIS').AsString = 'T')
  or (qrrastgeleHasta.FieldByName('YR_KONJ_GIS').AsString = 'T') then
    Exit('Yüksek');

  if (qrrastgeleHasta.FieldByName('OR_28_31_HAFTA').AsString = 'T')
  or (qrrastgeleHasta.FieldByName('OR_IUGR').AsString = 'T')
  or (qrrastgeleHasta.FieldByName('OR_1000_1500_GR').AsString = 'T')
  or (qrrastgeleHasta.FieldByName('OR_KONJ_BESLENME').AsString = 'T') then
    Exit('Orta');

  if (qrrastgeleHasta.FieldByName('DR_32_35_HAFTA').AsString = 'T')
  or (qrrastgeleHasta.FieldByName('DR_IUGR_35').AsString = 'T')
  or (qrrastgeleHasta.FieldByName('DR_TERM').AsString = 'T') then
    Exit('Düşük');
end;
...
begin
  ...
  MaxHafta := qrrastgeleHasta.FieldByName('HAFTA_NO').AsInteger;
...
begin
  ...
  Risk := CalcRiskSeviye;
  qrrastgeleHasta.FieldByName('RISK_SEVIYE').AsString := Risk;

  qrrastgeleHasta.Post;

  ShowMessage(
    'Kayıt alındı' + sLineBreak +
    'Hafta: ' + qrrastgeleHasta.FieldByName('HAFTA_NO').AsString + sLineBreak +
    'Risk: ' + Risk
  );

  SetHaftaAktif(GetAktifHafta);
end;
...
begin
  ...
  qrrastgeleHasta.ParamByName('DOSYA_NO').AsInteger := TEST_DOSYA_NO;
  qrrastgeleHasta.ParamByName('PROTOKOL_NO').AsInteger := TEST_PROTOKOL_NO;
  qrrastgeleHasta.Open;

  AktifHafta := GetAktifHafta;

  // Eğer hiç kayıt yoksa → 1. hafta oluştur
  if qrrastgeleHasta.IsEmpty then
  begin
    qrrastgeleHasta.Append;
    qrrastgeleHasta.FieldByName('DOSYA_NO').AsInteger := TEST_DOSYA_NO;
    qrrastgeleHasta.FieldByName('PROTOKOL_NO').AsInteger := TEST_PROTOKOL_NO;
  ...
  begin
    // Aktif haftaya git
    if not qrrastgeleHasta.Locate('HAFTA_NO', AktifHafta, []) then
    begin
      // DB'de yoksa yeni kayıt aç
      qrrastgeleHasta.Append;
      qrrastgeleHasta.FieldByName('DOSYA_NO').AsInteger := TEST_DOSYA_NO;
      qrrastgeleHasta.FieldByName('PROTOKOL_NO').AsInteger := TEST_PROTOKOL_NO;
      qrrastgeleHasta.FieldByName('HAFTA_NO').AsInteger := AktifHafta;
      qrrastgeleHasta.FieldByName('IZLEM_TARIHI').AsDateTime := Date;
    end;
  end;
